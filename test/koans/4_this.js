...
describe('JS this context behavior', function () {
  ...
  it('looping with a callback', function (done) {
    ...
    loop([3, 4, 5], function(value){
      ...
      assert.isTrue(this instanceof Array, "The context should be the full array.");
      if (num === 5) { done(); }
    });
  });
  ...
  it('order of precedence can be proved', function () {
    ...
    assert.throws(sayHey, ___, 'in strict mode, so this is null because of default binding rule');
