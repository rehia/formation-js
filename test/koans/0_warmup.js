...
describe('Koans warm-up', function () {
  ...
  it('is a koan', function () {
    var expected_result = 2; // if you feel that the expected result is bad, change it !
  ...
  it('should be solved one after the other', function () {
    ...
    assert.equal(palindrome, 'radar', 'turn the red light into green by solving the koan, but one after the other');
  });

  it('is not about finding the answer', function () {
    assert.isTrue(true, 'a koan is not only about finding the result, it’s about understanding what’s happening');
  });

  it('could be helpful to fill the blanks', function () {
    assert.include('hey !', 'hey', 'when you meet a blank, try to guess how to fill it ! You’re here to learn, so don’t be afraid to fail !');
  });
